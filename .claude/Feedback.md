...
### ADDF 本体への追加候補: PlanTemplate.md（2026-07-03、計画02）

- **背景**: 計画書 10〜17 の8本が独立起草にもかかわらず同一構造（実装状況 / 目的 / 現状の挙動 / 変更内容 / 影響範囲 / テスト方針 / 破壊的変更の許容範囲 / 要オーナー確認 / 見積もり）に収束した。ADDF には ProgressTemplate.md はあるが Plan のテンプレートが無い
- **提案**: `.claude/templates/PlanTemplate.md` を ADDF 本体に追加し、CLAUDE.md 骨格プランニング・addf-init から参照させる。検討スタブ用の簡略 variant（分かっていること / 未解決の問い / 着手のトリガー）も併記。「AI実装の見積もり」欄は個人設定由来のため任意セクション扱いを推奨
- **保留判断**: 「構想→計画書群への並列分解」ワークフロー自体の昇格は n=1 のため見送り（他プロジェクトでの再現例が出てから）
