...
### General
- Feat: チャンネルのノートを連合できるように (チャンネル単位のオプトイン)
	- チャンネル設定の「ノートの連合」で「連合する (ひかえめ)」(ホーム扱いで連合し、リモートの公開タイムラインには表示されない) と「連合する (公開)」を選択できます。既定は従来どおり連合しません。
	- 連合するチャンネルのノートには、リモートの読者向けに本文末尾へチャンネル名とURLが付記されます (リモートユーザーがこのURLをワードミュートすることでチャンネル単位のミュートができます)。
	- 連合しているチャンネルはチャンネル名の先頭に 🪐 が表示されます。
