...
- AP 関連コードは約6,400行 + `apRendererService` 呼び出し83箇所 + Note/User エンティティのカラム（uri / inbox / publicKey 等）に浸潤。削除すると upstream 追従（MISTEMS は develop から毎回作り直す統合方式）の難度が跳ね上がる
- **推奨は「設定 'none' + 再有効化防止パッチ + nodeinfo 直リンク/enqueue 抑止の小パッチ」**
...
- MISTEMS は mistems-main を毎回 develop から作り直すため、閉鎖パッチも機能ブランチ化して `main-統合.sh` に登録する必要がある（mistems-integration-flow.md 参照）
...
- 関連: [mistems-integration-flow.md](mistems-integration-flow.md)
